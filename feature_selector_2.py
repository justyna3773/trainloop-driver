...
#             reservoir_path = (
#                 f"{self.base_path}/logs/spca_corr_attn_all_{self.args.model_name}/fcorr_reservoir_final.npz"
#             )
...
#         min_prestd: float = 0.0,             # drop columns with std < threshold before centering
#         clip_z: Optional[float] = 5.0,       # optionally clip centered values to [-clip, clip]
#         # --- aggregation knobs ---
...
#         Xk = X[:, keep].astype(np.float64, copy=False)
#         mu = np.nanmean(Xk, axis=0, keepdims=True)
#         Zk = Xk - mu
#         if clip_z is not None:
#             clip_val = float(clip_z)
#             Zk = np.clip(Zk, -clip_val, clip_val)
...
from utils import FEATURE_NAMES
from spca_selector import SimpleSPCASelector, simple_spca_weights
...
class FeatureSelectionTrainer_bp:
    """
    Trainer + feature-selection helper using:
      - Attention logs
      - SPCA / correlation / IG on a reservoir .npz
    ...
    def select_from_reservoir(
        ...
        # --- SPCA weights via simplified selector ---
        simple_selector = SimpleSPCASelector(
            feature_names=self.feature_names if self.feature_names is not None else names,
            n_components=spca_components,
            alpha=spca_alpha,
            ridge_alpha=0.01,
            top_k=k,
        )
        w_spca, comps = simple_spca_weights(
            X_use,
            n_components=simple_selector.n_components,
            alpha=simple_selector.alpha,
            ridge_alpha=simple_selector.ridge_alpha,
        )
        print(f'WSPCA: {w_spca}')

        # Correlations (if enabled and y available)
        r = None
        if compute_correlation and (y_use is not None):
            r = self._corr_feature_target(X_use, y_use)
        ...
        df["spca_w_pct"] = df["spca_w"] / max(df["spca_w"].sum(), 1e-12)
        df["ig_pct"] = df["ig"] / max(df["ig"].sum(), 1e-12) if compute_ig else np.zeros_like(df["ig"])
        if r is not None:
            df["abs_corr_pct"] = df["abs_corr"] / max(df["abs_corr"].sum(), 1e-12)
        ...
        if verbose:
            ...
            if compute_ig and df_rank_ig is not None:
                print(f"\nTop-{k_show} cech wg IG (akcje):")
                for i, row in df_rank_ig.head(k_show).iterrows():
                    print(
                        f"  {i+1:>2}. {row['feature']} (#{int(row['idx'])})  ig={row['ig']:.6e}"
        ...
        if m == "spca":
            ...
            if not compute_ig:
                raise RuntimeError("IG scores were disabled (compute_ig=False).")
            scores = df["ig"].to_numpy()
        else:
            if not compute_correlation:
                raise RuntimeError("Correlation scores are disabled (compute_correlation=False).")
            if r is None:
                raise RuntimeError(
                    "Reservoir does not have y; cannot compute correlation-based scores."
                )
        ...
        if plot:
            self._plot_reservoir_rankings(
                df_rank_spca=df_rank_spca,
                df_rank_corr=df_rank_corr if compute_correlation else None,
                df_rank_ig=df_rank_ig if (compute_ig and df_rank_ig is not None) else None,
                top_k=min(top_k_plot, N),
                save_prefix=save_prefix,
            )
...
from utils import FEATURE_NAMES
from spca_selector import SimpleSPCASelector
...
class FeatureSelectionTrainer:
    ...
    def select_from_reservoir(
        ...
        # --- SPCA weights via SimpleSPCASelector class ---
        simple_selector = SimpleSPCASelector(
            feature_names=self.feature_names if self.feature_names is not None else names,
            n_components=spca_components,
            alpha=spca_alpha,
