...
def visualize_image_attr(
    ...
    if ImageVisualizationMethod[method] == ImageVisualizationMethod.original_image:
        ...
        plt_axis_original_image = plt_axis.imshow(original_image, cmap='Blues')
    else:
        ...
        if ImageVisualizationMethod[method] == ImageVisualizationMethod.heat_map:
            heat_map = plt_axis.imshow(norm_attr, vmin=vmin, vmax=vmax)
        elif (
    ...
    if show_colorbar:
        ...
        colorbar_axis = axis_separator.append_axes("right", size="5%", pad=0.5)
        if heat_map:
            plt_fig.colorbar(heat_map, orientation="vertical", cax=colorbar_axis)
        # else:
        #     colorbar_axis.axis("off")
        if plt_axis_original_image:
            plt_fig.colorbar(plt_axis_original_image, orientation="vertical", cax=colorbar_axis)
    if title:
    ...
    # plt.yticks(range(15), range(15), rotation=0)
    # plt_axis.ylabel('Timesteps')
    # plt_axis.xlabel('Features')
    plt_axis.set_ylabel('Features')
    plt_axis.set_xlabel('Timesteps')

    # plt_axis.set_yticklabels([list(range(15))])
    # plt_axis.set_xticklabels([Utils.FEATURE_NAMES])
...
def visualize_image_attr_multiple(
    ...
    **kwargs: Any,
    ...
    # else:
    #     plt_fig = Figure(figsize=fig_size)
    plt_axis = plt_fig.subplots(len(methods), 1)
    ...
    for i in range(len(plt_axis)):
        plt.sca(plt_axis[i])
        plt.yticks(list(range(len(Utils.FEATURE_NAMES))), Utils.FEATURE_NAMES)
        plt.xticks(list(range(attr.shape[1])), list(range(0, -int(attr.shape[1]), -1)))
